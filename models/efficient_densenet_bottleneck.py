...
class _EfficientDensenetBottleneckFn(Function):
    def __init__(self, buffr_1,
        ...
        self.efficient_conv = EfficientConv2d(stride, padding, dilation, groups)
    ...
    def forward(self, bn_weight, bn_bias, conv_weight, input):
        ...
        bn_output = self.efficient_batch_norm.forward(bn_weight, bn_bias, input)
    ...
    def backward(self, grad_output):
        ...
        # Recompute BN
        bn_output = self.efficient_batch_norm.forward(self.bn_weight, self.bn_bias, self.input)
        relu_output = self.efficient_relu.forward(bn_output)
        ...
        bn_weight_grad, bn_bias_grad, bn_grad_output = self.efficient_batch_norm.backward(
                self.bn_weight, self.bn_bias, self.input, relu_grad_output)
        ...
        return (bn_weight_grad, bn_bias_grad, conv_weight_grad, bn_grad_output)


class EfficientDensenetBottleneck(nn.Module):
    def __init__(self, buffr_1, num_input_channels, num_output_channels):
    ...
    def forward(self, input):
        fn = _EfficientDensenetBottleneckFn(self.buffr_1,
            self.norm_running_mean, self.norm_running_var,
            stride=1, padding=0, dilation=1, groups=1,
            training=self.training, momentum=0.1, eps=1e-5)
        return fn(self.norm_weight, self.norm_bias, self.conv_weight, input)
